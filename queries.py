...
def Q_1(conn, execution_time):
    ...
    query = """ SELECT 
    p.name, 
    AVG(s.statsbomb_xg) as average_xg_score
FROM 
    shot s
JOIN 
    player p ON (s.player_id = p.player_id AND s.match_id = p.match_id)
JOIN 
    matchs m ON s.match_id = m.match_id
JOIN 
...
def Q_3(conn, execution_time):
    ...
    query = """SELECT 
    p.name, 
    COUNT(s.event_id) as number_of_first_time_shots
FROM 
    shot s
JOIN 
    player p ON (s.player_id = p.player_id AND s.match_id = p.match_id)
...
HAVING 
    COUNT(s.event_id) > 0
ORDER BY 
    number_of_first_time_shots DESC;
 """
...
def Q_4(conn, execution_time):
    ...
    query = """ SELECT 
    t.team_name, 
    COUNT(p.event_id) as number_of_passes
FROM 
    pass p
JOIN 
    team t ON (p.team_id = t.team_id AND p.match_id = t.match_id)
JOIN 
    matchs m ON p.match_id = m.match_id
JOIN 
    competition c ON (m.competition_id = c.competition_id 
    AND m.season_id = c.season_id)
WHERE 
...
def Q_5(conn, execution_time):
    ...
    query = """ SELECT 
    p.recipient_name, 
    COUNT(p.event_id) as number_of_passes_received
FROM 
    pass p
JOIN 
    player pl ON (p.recipient_id = pl.player_id AND p.match_id = pl.match_id)
JOIN 
    matchs m ON p.match_id = m.match_id
JOIN 
    competition c ON (m.competition_id = c.competition_id 
...
WHERE 
    ...
    c.season_name = '2003/2004'
GROUP BY 
    p.recipient_name
HAVING 
    COUNT(p.event_id) > 0
ORDER BY 
    number_of_passes_received DESC;
"""
...
FROM 
    shot s
JOIN 
    team t ON (s.team_id = t.team_id AND s.match_id = t.match_id)
JOIN 
    matchs m ON s.match_id = m.match_id
JOIN 
    competition c ON (m.competition_id = c.competition_id 
...
def Q_7(conn, execution_time):
    ...
    query = """ SELECT 
    pl.name, 
    COUNT(p.event_id) as number_of_through_balls
FROM 
    pass p
JOIN 
    player pl ON (p.player_id = pl.player_id AND p.match_id = pl.match_id)
JOIN 
    matchs m ON p.match_id = m.match_id
JOIN 
    competition c ON (m.competition_id = c.competition_id 
...
WHERE 
    c.competition_name = 'La Liga' AND 
    c.season_name = '2020/2021' AND 
    (p.technique_id = 108 OR p.technique_name = 'Through Ball')
GROUP BY 
    pl.name
HAVING 
    COUNT(p.event_id) > 0
...
def Q_9(conn, execution_time):
    ...
    query = """SELECT 
    p.name, 
    COUNT(d.event_id) as number_of_successful_dribbles
FROM 
    dribble d
JOIN 
    player p ON (d.player_id = p.player_id AND d.match_id = p.match_id)
JOIN 
    matchs m ON d.match_id = m.match_id
JOIN 
    competition c ON (m.competition_id = c.competition_id 
    AND m.season_id = c.season_id)
WHERE 
...
def Q_10(conn, execution_time):
    ...
    query = """ SELECT 
    p.name, 
    COUNT(dp.event_id) as number_of_times_dribbled_past
FROM 
    dribbled_past dp
JOIN 
    player p ON dp.player_id = p.player_id
JOIN 
    matchs m ON dp.match_id = m.match_id
JOIN 
    competition c ON (m.competition_id = c.competition_id 
    AND m.season_id = c.season_id)
WHERE 
...
HAVING 
    COUNT(dp.event_id) > 0
ORDER BY 
    number_of_times_dribbled_past ASC;
"""
